...
Vérifie la création et les relations entre :
    • Role ;
    • User ;
    • Client ;
    • Contract ;
...
class TestModels(unittest.TestCase):
    """Suite de tests garantissant l’intégrité des modèles."""
    ...
    def test_create_user(self):
        """Un utilisateur doit pouvoir être créé et retrouvé par e‑mail."""
        user = User(
    ...
    def test_create_client(self):
        """La relation Client → Commercial doit être fonctionnelle."""
        commercial = User(
        ...
        queried_client = (
            self.session.query(Client).filter_by(
    ...
    def test_create_contract(self):
        """Un contrat signé doit être enregistré avec les bons montants."""
        commercial = User(
        ...
        queried_contract = (
            self.session.query(Contract).filter_by(client_id=client.id).first()
        )
        self.assertIsNotNone(queried_contract)
        self.assertTrue(queried_contract.is_signed)
        self.assertEqual(queried_contract.total_amount, 10_000.0)

    def test_create_event(self):
        """La relation Event → Support / Contract doit être cohérente."""
        commercial = User(
        ...
        self.assertIsNotNone(queried_event)
        self.assertEqual(queried_event.support.first_name, "David")
        self.assertEqual(
            queried_event.location,
